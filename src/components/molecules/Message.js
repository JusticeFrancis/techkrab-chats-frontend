...
export const User = ({ data }) => (
  <div className=" flex items-center my-2 justify-end  ">
    <div className=" w-3/5  space-x-2 bg-[#075e54] text-white rounded-tl-lg  rounded-b-lg py-1 px-2 ">
      {data.has_file && (
        <div>
          {data.file.type.substr(0, 5) === "image" ? (
            <div>
              <img
           ...
           <>
           {data.file.type === 'audio/webm;codecs=opus' ? (
             <audio controls src={URL.createObjectURL(data.file)}>
             </audio>
           ):(
...
export const Guest = ({ data }) => (
  <div className=" flex items-center my-2 ">
    <div className=" w-3/5  space-x-2  bg-[#435a64] text-white rounded-tr-lg  rounded-b-lg py-1 px-2 ">
      {data.has_file && (
        <div>
          {data.file_extension.substr(0, 5) === "image" ? (
            <div>
              <img
                src={
                  "https://techkrab-socket-backend-production.up.railway.app/tmp/" +
                  data.filename
                }
            ...
            <>
              {data.file_extension === "audio/webm;codecs=opus" ? (
                <div className="flex items-center space-x-2">
                  <audio controls src={"https://techkrab-socket-backend-production.up.railway.app/tmp/" +
                  data.filename}>
                  </audio>
                </div>
              ) : (
